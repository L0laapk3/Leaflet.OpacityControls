...
L.Control.opacitySlider = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        this.options.labels.sort((a, b) => a.position < b.position);
        ...
        this.options.labels.filter(l => l.layers).forEach((label, i, arr) => {
            ...
            label.layers.forEach(layer => {
                ...
                if (!container) {
                    var $cont = $("<div style = 'z-index: " + (arr.length - i) + "' class='leaflet-opacity-stack'>");
        ...
        function update(value) {
            ...
            if (i >= 0) aboveI = i;
            var snap;
            if      (belowI !== undefined && value - _this.options.labels[belowI].position <= (_this.options.labels[belowI].gravitate || _this.options.gravitate)) { snap = aboveI = belowI; }
            else if (aboveI !== undefined && _this.options.labels[aboveI].position - value <= (_this.options.labels[aboveI].gravitate || _this.options.gravitate)) { snap = belowI = aboveI; }
            ...
            _this.options.labels.forEach(label => {
                ...
                if (label.containers[0].style.opacity != newValue) {
                    ...
                    label.layers.forEach(layer => {
                        ...
                        if (Object.values(layer._opacities).some(v => v < 0.01) && map.hasLayer(layer) && !layer._t1)  layer._t1 = setTimeout(() => { if (map.hasLayer(layer)) map.removeLayer(layer); layer._t1 = undefined }, 250);
